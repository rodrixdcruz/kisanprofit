...
def create_farm(payload: FarmCreate, user: User = Depends(require_writable_user),
...
def update_farm(farm_id: int, payload: FarmCreate, user: User = Depends(require_writable_user),
                db: Session = Depends(get_db)):
    farm = db.query(Farm).filter(Farm.id == farm_id, Farm.owner_id == user.id).first()
    if not farm:
        raise HTTPException(404, "Farm not found")
...
def delete_farm(farm_id: int, user: User = Depends(require_writable_user), db: Session = Depends(get_db)):
    farm = db.query(Farm).filter(Farm.id == farm_id, Farm.owner_id == user.id).first()
    if not farm:
        raise HTTPException(404, "Farm not found")
    db.delete(farm)
